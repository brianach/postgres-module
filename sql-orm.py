...
#artist = session.query(Artist).filter_by(Name="Pearl Jam").first()
#print(artist.ArtistId, artist.Name, sep=" | ")

# Query 4 - select using "ArtistId" from the "Artist" table
#artist = session.query(Artist).filter_by(ArtistId=118).first()
#print(artist.ArtistId, artist.Name, sep=" | ")


# Query 5 - select all records for "Pearl Jam" "Artist Id" from the "Album" table
#albums = session.query(Album).filter_by(ArtistId=118)
#for album in albums:
#    print(album.AlbumId, album.Title, album.ArtistId, sep=" | ")


# Query 6 - select all records from "Track" table where the composer is "Pearl Jam"
tracks = session.query(Track).filter_by(Composer="Pearl Jam")
for track in tracks:
    print(
        track.TrackId, 
        track.Name, 
